...
def preprocess_mimic_extract(df: pd.DataFrame, task: str, static_features: List[str]) -> pd.DataFrame:
    ...
    Specifically, this includes flattening the data and imputing missing values."""
    # Remove Ys and static features; this allows us to use the same preprocessing code
    # to flatten the time-varying features as in the MIMIC provided notebooks.
    ...
    flattened_df = df.pivot_table(index=['subject_id', 'hadm_id', 'icustay_id'], columns=['hours_in'])

    # Flatten the columns prior to joining and clean up column names
    flattened_df.columns = flattened_df.columns.to_flat_index()
    flattened_df.columns = ["_".join(str(x).replace(' ', '_') for x in c) for c in flattened_df.columns]

    # All of these targets do not vary based on time, so we can drop the 'hours_in'
    # level (which is not present in the flattened DataFrame index either after
    # flattening) and merge on the remaining 3-level index with
    # levels (subject_id, hadm_id, icustay_id).
    NON_TIME_VARYING_TARGETS = ('los_3', 'los_7', 'mort_hosp', 'mort_icu')
    assert Ys.name in NON_TIME_VARYING_TARGETS, f"sanity check that label {Ys.name} is not time-varying."

    def _drop_hours_in_index(df_in: Union[pd.Series, pd.DataFrame]) -> Union[pd.Series, pd.DataFrame]:
        """Helper function to remove the hours_in index level."""
        assert df_in.index.names == ['subject_id', 'hadm_id', 'icustay_id', 'hours_in']
        df_in = df_in.droplevel('hours_in')
        return df_in[~df_in.index.duplicated(keep='first')]

    # Drop the hours_in from index and then drop duplicate indices (the values are the same
    # at every value of hours_in for each unique (subject_id, hadm_id, icustay_id) index,
    # since the label and static values are all not time-varying.
